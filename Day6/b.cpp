...
int main()
{
    ...
    // Only need to change the offsets.
    for (; indexStartOfPacket < line.length() - 13; ++indexStartOfPacket)
    {
        std::string currBuf{line.substr(indexStartOfPacket, 14)};
    ...
    std::cout << indexStartOfPacket + 14 << " characters processed before start-of-packet." << std::endl;

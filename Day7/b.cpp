...
constexpr uint64_t maxSpace{70'000'000};
constexpr uint64_t minUnusedSpace{30'000'000};
...
uint64_t updateSizes(FileSystemNode* node)
{
    if (node->type == file)
    {
        return node->size;
...
void findBestDirectoryToDelete(FileSystemNode* node, uint64_t spaceToFree, uint64_t& sizeOfDirToDelete)
{
    if (node->type == file)
    {
        return;
    ...
    {
        if (node->size > spaceToFree && node->size < sizeOfDirToDelete)
        {
            sizeOfDirToDelete = node->size;
        }
    }

    for(auto& c : node->children)
    {
        findBestDirectoryToDelete(&c, spaceToFree, sizeOfDirToDelete);
    }
}
...
int main()
{
    ...
    // First populate all sizes.
    updateSizes(&rootfs);

    // How much space should we free?
    uint64_t spaceToFree{minUnusedSpace - (maxSpace - rootfs.size)};

    uint64_t sizeOfDirToDelete = maxSpace;
    findBestDirectoryToDelete(&rootfs, spaceToFree, sizeOfDirToDelete);

    std::cout << "The solution to the puzzle is " << sizeOfDirToDelete << std::endl;
